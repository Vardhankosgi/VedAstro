    ...
    public static partial class APITools
    {
        ...
        public static HttpResponseData MessageJson<T>(string statusResult, T payload, HttpRequestData req, string contentType = MediaTypeNames.Application.Json)
        {
            ...
            else
            {
                ...
                if (payload != null)
                {
                    if (payload is XElement payloadXElement)
                    {
                        finalPayloadJson["Payload"] = payloadXElement.ToString();
                    }
                    else
                    {
                        var strVal = payload.ToString() ?? "";
                        var cleanStr = strVal.Trim();
                        if ((cleanStr.StartsWith("{") && cleanStr.EndsWith("}")) || (cleanStr.StartsWith("[") && cleanStr.EndsWith("]")))
                        {
                            finalPayloadJson["Payload"] = JToken.Parse(strVal);
                        }
                        else
                        {
                            finalPayloadJson["Payload"] = strVal;
                        }
                    }
                }
            }
        ...
        public static TableClient GetTableClientFromTableName(string tableName)
        {
            ...
            //get connection
            var _tableServiceClient = new TableServiceClient(storageAccountKey);
